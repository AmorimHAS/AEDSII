...
class GameBinarioHDRE {
    ...
    GameBinarioHDRE() {
        ...
        this.price = 0.0f;
        this.supportedLanguages = new String[NoArrayListHDRE.MAX_INNER_ARRAY];
        this.supportedLanguagesCount = 0;
        this.metacriticScore = -1;
        this.userScore = -1.0f;
        this.achievements = 0;
        this.publishers = new String[NoArrayListHDRE.MAX_INNER_ARRAY];
        this.publishersCount = 0;
        this.developers = new String[NoArrayListHDRE.MAX_INNER_ARRAY];
        this.developersCount = 0;
        this.categories = new String[NoArrayListHDRE.MAX_INNER_ARRAY];
        this.categoriesCount = 0;
        this.genres = new String[NoArrayListHDRE.MAX_INNER_ARRAY];
        this.genresCount = 0;
        this.tags = new String[NoArrayListHDRE.MAX_INNER_ARRAY];
        this.tagsCount = 0;
    }

    GameBinarioHDRE(int id, String name, String releaseDate, int estimatedOwners, float price,
...
class HashRehash {
    ...
    public HashRehash(int m) {
        this.m = m;
        this.tabela = new GameBinarioHDRE[this.m];
        for (int i = 0; i < m; i++) {
            tabela[i] = null;
        }
    }
    ...
    public boolean inserir(GameBinarioHDRE game) {
        boolean resp = false;
        if (game != null) {
            int indice = 0;
    ...
    public boolean pesquisar(String name) {
        ...
        if (tabela[pos] != null && tabela[pos].name.equals(name)) {
            ...
            pos = reh(indice);
            if (tabela[pos].name.equals(name)) {
                posFinal = pos;
                resp = true;
            }
        }
        ...
        if (resp) {
            System.out.println(name + ":  (Posicao: " + posFinal + ") SIM");
            TabelaHashDiretaComRehash.logWriter.println(name + ":  (Posicao: " + posFinal + ") SIM");
        } else {
            System.out.println(name + ":  (Posicao: " + posFinal + ") NAO");
            TabelaHashDiretaComRehash.logWriter.println(name + ":  (Posicao: " + posFinal + ") NAO");
        }
        return resp;
    }
}
...
public class TabelaHashDiretaComRehash {
    ...
    public static void main(String[] args) {
        ...
        // Arquivo log
        try {
            logWriter = new PrintWriter(new FileWriter("885637_hashRehash.txt"));
        } catch (IOException e) {
        ...
        // Criando a árvore
        HashRehash hash = JogosDigitadosHashRehash.inicializacao(ids, tam);
            ...
            try {
                ...
                while (sc.hasNextLine() && indiceEncontrado == -1) {
                    ...
                    if (indiceEncontrado != -1) {
                        ...
                        String[] supportedLanguages = new String[NoArrayListHDRE.MAX_INNER_ARRAY];
                        ...
                        String[] publishers = new String[NoArrayListHDRE.MAX_INNER_ARRAY];
                        int publishersCount = capturaUltimosArryays(linha, publishers);
                        String[] developers = new String[NoArrayListHDRE.MAX_INNER_ARRAY];
                        int developersCount = capturaUltimosArryays(linha, developers);
                        String[] categories = new String[NoArrayListHDRE.MAX_INNER_ARRAY];
                        int categoriesCount = capturaUltimosArryays(linha, categories);
                        String[] genres = new String[NoArrayListHDRE.MAX_INNER_ARRAY];
                        int genresCount = capturaUltimosArryays(linha, genres);
                        String[] tags = new String[NoArrayListHDRE.MAX_INNER_ARRAY];
                        int tagsCount = capturaUltimosArryays(linha, tags);

                        GameBinarioHDRE jogo = new GameBinarioHDRE(id, name, releaseDate, estimatedOwners, price,
